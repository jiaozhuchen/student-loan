    ...
    <div class="table-container">
      <el-table ref="homeAdvertiseTable"
        ...
        <el-table-column label="广告位置" width="120" align="center">
          ...
          <template slot-scope="scope">{{arrTypeKeyName[scope.row.type]}}</template>
        </el-table-column>
...
<script>
  import {fetchList,updateStatus,deleteHomeAdvertise,fetchCarouselTypeList} from '@/api/homeAdvertise';
      ...
      getCateList(){
        fetchCarouselTypeList().then(response=>{
          ...
          for(let i=0;i<cateList.length;i++){
            let cate = cateList[i];
            this.arrTypeKeyName[cate.key] = cate.value
            this.typeOptions.push({label:cate.value,value:cate.key});
          }
          this.defaultType=cateList[0].id;
        })
      }
    }
  }
