...
import { getLoanApply, createLoanApply } from "@/api/loanApply";
import {getStudentInformation} from '@/api/student';
...
export default {
  ...
  created() {
    if (this.isEdit) {
      ...
      this.loanApply = Object.assign({}, defaultLoanApply);
    }
  },
  methods: {
    resetForm(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.$confirm('是否提交数据', '提示', {
            ...
            createLoanApply(this.loanApply).then(response=>{
              this.$message({
                type: 'success',
                message: '提交成功',
                duration:1000
