...
<script>
  import {getBrandList, updateBrand} from '@/api/repair'
  ...
  export default {
    ...
    methods: {
      ...
      handleUpdate(index, row) {
        this.$router.push({path: '/repair/updateCoBrand', query: {id: row.id}})
      },
      handleDelete(index, row) {
        this.$confirm('是否要删除该品牌', '提示', {
          ...
          updateBrand(row.id,{state:0}).then(response => {
            this.$message({
              message: '删除成功',
              type: 'success',
              duration: 1000
      ...
      addBrand() {
        this.$router.push({path: '/repair/addCoBrand'})
      }
    }
  }
</script>
